...
export default function Create({ activity, batches }) {
    const { data, setData, post, processing, errors } = useForm({
        name: '',
        description: '',
        jenis_absen: ['Manual'], // Default value
        activity_batch_id: '',
    });

    const submit = (e) => {
        e.preventDefault();
        post(route('attendance.store', activity.id), {
            onSuccess: () => {
                Swal.fire({
                    title: 'Berhasil!',
                    text: 'Jenis absen berhasil dibuat.',
                    icon: 'success',
                    timer: 1500,
    ...
    return (
        <AcaraLayout activity={activity} title="Tambah Jenis Absen">
            <div className="min-h-screen bg-gray-50 py-4 sm:py-6 lg:py-8 px-3 sm:px-4 lg:px-6">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
                        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-4 sm:px-6 py-3 sm:py-4">
                            <h3 className="text-lg sm:text-xl font-bold text-white">Tambah Jenis Absen</h3>
                        </div>
                        ...
                        <div className="p-4 sm:p-6">
                            <form onSubmit={submit} className="space-y-4 sm:space-y-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="jenis_absen" className="block text-sm font-semibold text-gray-700 mb-2">
                                        ...
                                        <div className="space-y-2 border p-3 rounded-lg bg-gray-50">
                                            {[
                                                ...
                                                { value: 'Mandiri', label: 'Mandiri (Tombol)' },
                                                { value: 'QR Manual', label: 'QR Code (Scan oleh Panitia)' },
                                                { value: 'QR Mandiri', label: 'QR Code (Scan oleh Peserta)' }
                                            ].map((option) => (
                                                <label key={option.value} className="flex items-center space-x-3 cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                        value={option.value}
                                                        checked={data.jenis_absen.includes(option.value)}
                                                        onChange={(e) => {
                                                            const checked = e.target.checked;
                                                            if (checked) {
                                                                setData('jenis_absen', [...data.jenis_absen, option.value]);
                                                            } else {
                                                                setData('jenis_absen', data.jenis_absen.filter(t => t !== option.value));
                                                            }
                                                        }}

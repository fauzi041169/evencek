...
class ActivityPreparationController extends Controller
{
    ...
    public function updateSettings(Request $request, $activityId)
    {
        $activity = Activity::findOrFail($activityId);
        ...
        $validated = $request->validate([
            'detail_description_visible' => 'sometimes|boolean',
            'detail_gallery_visible' => 'sometimes|boolean',
            'detail_comments_visible' => 'sometimes|boolean',
            'detail_participants_visible' => 'sometimes|boolean',
            'show_gallery' => 'sometimes|boolean',
            'description_visible' => 'sometimes|boolean',
            'enable_comments' => 'sometimes|boolean',
            'rundown_visible' => 'sometimes|boolean',
            'participants_visible' => 'sometimes|boolean',
            'materials_visible' => 'sometimes|boolean',
            'speakers_visible' => 'sometimes|boolean',
            'id_card_visible' => 'sometimes|boolean',
            'certificate_visible' => 'sometimes|boolean',
        ]);

        if ($request->has('id_card_visible')) {
            $cardSettings = CardSettings::firstOrCreate(
                [
            ...
            if (is_string($printSettings)) {
                $printSettings = json_decode($printSettings, true) ?? [];
            }
            $printSettings['id_card_visible'] = (bool) $request->input('id_card_visible');
            $cardSettings->print_settings = $printSettings;
            $cardSettings->save();

            // Remove from validated so we don't try to update Activity model
            unset($validated['id_card_visible']);
        }

        if ($request->has('certificate_visible')) {
            $certSettings = CertificateSettings::firstOrCreate(
                [
                    'activity_id' => $activity->id,
                    'activity_batch_id' => null
                ],
            ...
            $printSettings = $certSettings->print_settings ?? [];
            if (is_string($printSettings)) {
                $printSettings = json_decode($printSettings, true) ?? [];
            }
            // Map 'certificate_visible' from request to 'download_card_visible' in DB
            $printSettings['download_card_visible'] = (bool) $request->input('certificate_visible');
            $certSettings->print_settings = $printSettings;
            $certSettings->save();

            unset($validated['certificate_visible']);
        }
